...
const circlesDir = path.join(__dirname, '../public/magiccircle/circles2');
...
files.forEach(file => {
  ...
  // Only process paths in the main <g> group, not in <defs>
  // Find the <g transform=...> section
  const gMatch = content.match(/<g transform[^>]*>([\s\S]*?)<\/g>/);
  if (!gMatch) {
    console.log(`${file}: No <g> group found, skipping`);
    return;
  }
  ...
  while ((match = pathDRegex.exec(gContent)) !== null) {
  ...
  // Calculate centered square viewBox in original coordinates
  ...
  const origVbX = centerX - size / 2;
  const origVbY = centerY - size / 2;

  // Transform: matrix(0.13333333,0,0,-0.13333333,0,666.66667)
  // x' = x * 0.13333333
  // y' = y * (-0.13333333) + 666.66667
  const scale = 0.13333333;

  // Transformed viewBox
  const vbX = origVbX * scale;
  const vbY = (origVbY + size) * (-scale) + 666.66667;
  const vbSize = size * scale;

  console.log(`${file}: bounds (${globalMinX.toFixed(0)}-${globalMaxX.toFixed(0)}, ${globalMinY.toFixed(0)}-${globalMaxY.toFixed(0)}) → ${vbSize.toFixed(2)}x${vbSize.toFixed(2)}`);

  // Update viewBox
  content = content.replace(/viewBox="[^"]*"/, `viewBox="${vbX.toFixed(2)} ${vbY.toFixed(2)} ${vbSize.toFixed(2)} ${vbSize.toFixed(2)}"`);

  // Update rect
  content = content.replace(/<rect[^>]*\/>/, `<rect x="${vbX.toFixed(2)}" y="${vbY.toFixed(2)}" width="${vbSize.toFixed(2)}" height="${vbSize.toFixed(2)}" fill="#1e1e20"/>`);
